...
function crm_generateAgreementAndPoa(matterId) {
  ...
  if (crm_hasAgreementPackage_(matterId)) {
    ...
    return { ok: false, code: "ALREADY_EXISTS", message: "Agreement + POA already exist for this matter." };
  }
  ...
  const createdDocs = [];
  const templates = (cfg_().TEMPLATES || {});
  const templateData = crm_buildTemplateData(client, matter);
  const useTemplateDocs = templates.AGREEMENT_DOC_ID && !templates.AGREEMENT_DOC_ID.startsWith("PUT_REAL_") &&
                          templates.POA_DOC_ID       && !templates.POA_DOC_ID.startsWith("PUT_REAL_");

  // Generate Agreement — use real Drive template copy when IDs are configured,
  // otherwise fall back to inline text generation.
  var agreementFile, agreementDocId, agreementDocUrl;
  if (useTemplateDocs) {
    agreementFile  = crm_generateDocFromTemplate_(
      templates.AGREEMENT_DOC_ID,
      "Agreement_" + matter.MATTER_ID + "_" + Date.now(),
      folderId,
      templateData
    );
    agreementDocId  = agreementFile.getId();
    agreementDocUrl = agreementFile.getUrl();
  } else {
    const agDoc_ = crm_createGoogleDocFromTemplate(
      "Agreement_" + matter.MATTER_ID + "_" + Date.now(),
      crm_buildAgreementText(client, matter),
      folderId
    );
    agreementDocId  = agDoc_.getId();
    agreementDocUrl = agDoc_.getUrl();
  }

  const agreementPdf = crm_exportDocToPdf(agreementDocId, folderId, "Agreement_" + matter.MATTER_ID + ".pdf");
  ...
  const agreementAdd = crm_addDocument({
    ...
    docUrl: agreementDocUrl,
    pdfUrl: agreementPdf.getUrl(),
    fileId: agreementDocId,
    createdBy: getActiveUserEmail_() || "system",
    notes: "Generated by CRM sign flow on " + nowIso_() + (useTemplateDocs ? " [template copy]" : " [inline text]"),
  });

  createdDocs.push(agreementAdd);

  // Generate POA
  var poaFile, poaDocId, poaDocUrl;
  if (useTemplateDocs) {
    poaFile  = crm_generateDocFromTemplate_(
      templates.POA_DOC_ID,
      "POA_" + matter.MATTER_ID + "_" + Date.now(),
      folderId,
      templateData
    );
    poaDocId  = poaFile.getId();
    poaDocUrl = poaFile.getUrl();
  } else {
    const poaDoc_ = crm_createGoogleDocFromTemplate(
      "POA_" + matter.MATTER_ID + "_" + Date.now(),
      crm_buildPoaText(client, matter),
      folderId
    );
    poaDocId  = poaDoc_.getId();
    poaDocUrl = poaDoc_.getUrl();
  }

  const poaPdf = crm_exportDocToPdf(poaDocId, folderId, "POA_" + matter.MATTER_ID + ".pdf");
  ...
  return {
    ...
    clientId: client.CLIENT_ID,
    folderUrl: matter.FOLDER_URL,
    generatedVia: useTemplateDocs ? "template_copy" : "inline_text",
    agreement: { docId: agreementAdd.docId, docUrl: agreementDocUrl, pdfUrl: agreementPdf.getUrl() },
    poa: { docId: poaAdd.docId, docUrl: poaDocUrl, pdfUrl: poaPdf.getUrl() },
  };
}

/**
 * Generates a Google Doc by copying a Drive template and replacing {{PLACEHOLDER}} tokens.
 * Falls back to crm_createGoogleDocFromTemplate (text-based) if templateId is missing/placeholder.
 *
 * @param {string} templateId  - Drive file ID of the template Google Doc
 * @param {string} docName     - Name for the generated copy
 * @param {string} folderId    - Drive folder ID where the copy should live
 * @param {Object} replacements - Map of PLACEHOLDER_KEY → value string
 * @returns {GoogleAppsScript.Drive.File} - The generated file (Google Doc)
 */
function crm_generateDocFromTemplate_(templateId, docName, folderId, replacements) {
  if (!templateId || templateId.startsWith("PUT_REAL_")) {
    throw new Error("crm_generateDocFromTemplate_: templateId is not configured. Set the real Doc ID in Config.gs TEMPLATES.");
  }

  const templateFile = DriveApp.getFileById(templateId);
  const destFolder   = DriveApp.getFolderById(folderId);
  const copy         = templateFile.makeCopy(docName, destFolder);
...
/**
 * Returns true when both AGREEMENT and POA documents exist for this matter (any status).
 * Use before generating to prevent duplicate doc sets.
 */
function crm_hasAgreementPackage_(matterId) {
  if (!matterId) return false;
  const docs = crm_listDocumentsByMatterId(matterId, { limit: 50 });
  const hasAgreement = docs.some(function(d) { return (d.TYPE || "").toUpperCase() === "AGREEMENT"; });
  const hasPoa       = docs.some(function(d) { return (d.TYPE || "").toUpperCase() === "POA"; });
  return hasAgreement && hasPoa;
}
